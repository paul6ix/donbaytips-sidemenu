...
.controller('MenuCtrl', function($http, $scope, $sce, $ionicScrollDelegate,$timeout,$rootScope){
	...
	$rootScope.$on('$cordovaPush:notificationReceived', function(event, notification){
		alert("result: " + JSON.stringify(notification));
	      ...
	      $http({
	          method  : 'POST',
	          url     : 'http://donbaystips.com.ng/pnfw/register/',
	...
	$http.get("http://donbaystips.com.ng/api/get_category_index/").then(
...
.controller('MainCtrl', function($http, $scope, $sce, $ionicScrollDelegate,$timeout, $localStorage, $ionicLoading){
	...
	$scope.doRefresh = function(){
		$scope.recent_posts = [];
		$http.get("http://donbaystips.com.ng/api/get_posts/").then(function(data){
			console.log(data);
	...
	$scope.loadMore = function(){
		...
		{
			$scope.lastTimer = new Date().getTime();
			$http.get("http://donbaystips.com.ng/api/get_posts/?offset="+$scope.offset)
			.then(function(data){
...
.controller('PostCtrl', function($scope, $http, $stateParams, $sce){
	...
	$http.get('http://donbaystips.com.ng/api/get_post/?id='+ $stateParams.postId).then(
		function(data){
...
.controller('CatCtrl', function($http, $scope, $sce, $stateParams){
	...
	$scope.doRefresh = function(){
		$http.get('http://donbaystips.com.ng/api/get_category_posts/?id=' + $stateParams.catId).then(
		function(data){
...
.controller('FavCtrl', function($http, $scope, $localStorage, $sce){
    ...
    $scope.Favorites.forEach(function(element, index, array){
      $http.get('http://donbaystips.com.ng/api/get_post/?id='+element)
